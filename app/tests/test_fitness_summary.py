...
class TestFitnessSummary:
    ...
    def test_daily_metrics_summary(self, auth_client):
        ...
        try:
            ...
            db.add(FitnessData(
                ...
                data=json.dumps({"rhr": 52, "avg_sleep_hrv": 72, "training_load": 45,
                                 "vo2max": 48, "training_load_ratio": 1.1, "tired_rate": 2.5}),
            ))
            db.add(FitnessData(
                user_id=user_id, date=today - timedelta(days=2),
                data_type="daily_metrics",
                data=json.dumps({"rhr": 54, "avg_sleep_hrv": 68, "training_load": 55,
                                 "vo2max": 47, "training_load_ratio": 1.2, "tired_rate": 3.0}),
            ))
            db.commit()
    ...
    def test_all_data_types(self, auth_client):
        ...
        try:
            ...
            db.add(FitnessData(
                user_id=user_id, date=today - timedelta(days=1),
                data_type="sleep",
                data=json.dumps({"total_duration_minutes": 450, "phases": {"deep_minutes": 100}}),
            ))
            db.add(FitnessData(
                user_id=user_id, date=today - timedelta(days=2),
                data_type="activity",
                data=json.dumps({
                    "name": "跑步", "duration_seconds": 2400,
                    "sport_name": "跑步",
                }),
            ))
            db.commit()
    ...
    def test_rhr_label(self, auth_client):
        ...
        try:
            db.add(FitnessData(
                user_id=user_id, date=date.today() - timedelta(days=1),
                data_type="daily_metrics",

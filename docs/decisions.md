...
### 近期优化（Agent 能力升级，1-2周）

| 方向 | 说明 | 优先级 |
|------|------|--------|
| **Multi-Agent 协作** | 拆分为运动知识Agent + 数据分析Agent + 计划生成Agent，各司其职，通过 Supervisor Agent 协调调度 | P0 |
| **LLM 训练建议** | 基于高驰真实训练数据（HRV、负荷、睡眠），大模型生成下一阶段训练计划，调用 `schedule_workout` 推送至高驰日历 | P0 |
| **意图路由** | 用户问"今天做什么训练"直接走计划Agent，问"蛋白粉怎么吃"走知识Agent，延迟降到1-3秒 | P1 |
| **Agentic RAG** | Agent 自主决定是否需要检索、检索什么关键词、几路检索，替代当前固定流程 | P1 |
| **对话中提取画像更新** | 用户说"我最近膝盖疼"，Agent 自动更新伤病史到 UserProfile | P2 |

### 中期升级（架构升级，2-4周）

| 方向 | 说明 | 优先级 |
|------|------|--------|
| **Plan-Execute Agent 模式** | 处理"制定8周减脂计划"这种复杂任务：先拆解为饮食计划+训练安排+进度追踪，再分步执行，支持用户中途干预 | P0 |
| **多轮对话记忆 v2.0** | 加对话摘要压缩（Summary Buffer Memory），长对话自动提取关键信息压缩为摘要，避免上下文窗口溢出、节省 token | P1 |
| **向量库升级** | Qdrant 单节点容器 + 离线 revision 索引 + 来源过滤；父子切片、稀疏向量和集群化由后续评测触发 | Demo 已实施 |
| **RAG 重排序（Rerank）** | 在双路检索 + RRF 融合后再加一层 Cross-Encoder 重排（如 BGE-Reranker），精准度再提升 5-10% | P2 |

...
| 2:40–4:10 | 填写画像后提问“结合我的目标和膝盖不适，今天怎么练？” | 个性化问题才进入 LangGraph Agent，按需读取画像、运动数据与 RAG。|
| 4:10–5:00 | 展示 `RAG_RETRIEVAL` 日志 | 可看到 revision、Dense/BM25 候选数、选择数和耗时。|
| 5:00–5:30 | 调用 `GET /api/sessions/{session_id}/agent-runs` | 展示一次 Agent 的工具顺序、状态与耗时；强调只存参数类型，不存问题和工具实参。|
| 5:30–6:00 | 打开 02、03 文档 | 说明离线构建 / 在线检索分离，revision alias 支持安全发布和回溯。|
...
个人数据走 MySQL，知识检索走 Qdrant；两者只在 Agent 编排层汇合。API、服务和仓储层各自承担 HTTP、业务流程、第三方/数据库访问职责。
...
重点测试包括：`test_direct_rag_router.py`（快速路径及 SSE 事件顺序）、`test_rag_service.py`（检索融合）、`test_database_session.py`（事务边界）和 `test_knowledge_enrichment.py`（离线切分与去重）。
